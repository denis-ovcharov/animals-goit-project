import{A as K,a as q,S as U,N as W,P as F,i as p,b as j}from"./assets/vendor-CyB7JiL4.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))o(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const l of i.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&o(l)}).observe(document,{childList:!0,subtree:!0});function s(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function o(n){if(n.ep)return;n.ep=!0;const i=s(n);fetch(n.href,i)}})();const H=document.getElementById("mobile-menu"),X=document.getElementById("data-burger-close"),J=document.getElementById("data-burger-open"),N=document.querySelector(".mobile-menu");function D(e){e.key==="Escape"&&H.classList.contains("is-open")&&R()}function V(e){e.target.closest("a")&&R()}function R(){H.classList.remove("is-open"),document.body.style.overflow="visible",document.removeEventListener("keydown",D),N.removeEventListener("click",V)}function Q(){H.classList.add("is-open"),document.body.style.overflow="hidden",document.addEventListener("keydown",D),N.addEventListener("click",V)}X.addEventListener("click",R);J.addEventListener("click",Q);const Z=[{question:"Я мрію про пухнастика! Що мені потрібно зробити, щоб забрати хвостика додому?",answer:`
  ...
  `,{onShow:t=>{const s=document.querySelector(".basicLightbox .pet-modal");s&&(s.style.overflow="auto"),document.body.style.overflow="hidden",t.element().querySelector(".pet-modal-close").addEventListener("click",()=>t.close()),t.element().querySelector(".pet-modal-btn").addEventListener("click",()=>t.close()),document.addEventListener("keydown",I)},onClose:()=>{document.body.style.overflow="visible",document.removeEventListener("keydown",I)}}),h.show()}function I(e){e.key==="Escape"&&h.close()}function ge(e){let t=e.replace(/\D/g,"");return t.length===10&&t.startsWith("0")&&(t="38"+t),t}function he(e){const t=j.create(`
    ...
    `,{onShow:s=>{document.body.style.overflow="hidden";const o=s.element(),n=o.querySelector(".modal-order-close-btn"),i=o.querySelector("[data-modal-order-form]"),l=o.querySelector("#user-name"),f=o.querySelector("#user-phone"),u=o.querySelector("#user-comment"),w=o.querySelector(".form-submit-btn");n.addEventListener("click",()=>s.close());const L=v=>{v.key==="Escape"&&s.close()};document.addEventListener("keydown",L),s._onEscClose=L,i.addEventListener("submit",async v=>{var A,P,O;v.preventDefault();const b=l.value.trim(),T=ge(f.value),M=u.value.trim();if(!e){p.error({message:"Не обрано тварину",position:"topRight"});return}if(!b||b.length>32){p.error({message:"Імʼя обовʼязкове (до 32 символів)",position:"topRight"}),l.focus();return}if(!/^[0-9]{12}$/.test(T)){p.error({message:"Телефон має містити 12 цифр (приклад: 380955559922)",position:"topRight"}),f.focus();return}if(M&&M.length>500){p.error({message:"Коментар не може перевищувати 500 символів",position:"topRight"}),u.focus();return}const Y={name:b,phone:T,animalId:e,comment:M||void 0};try{w.disabled=!0,B(),await ae(Y),p.success({title:"Успішно",message:"Заявку відправлено",position:"topRight"}),s.close()}catch(m){console.error(((A=m==null?void 0:m.response)==null?void 0:A.data)||m.message),p.error({title:"Помилка",message:((O=(P=m==null?void 0:m.response)==null?void 0:P.data)==null?void 0:O.message)||"Не вдалося відправити заявку",position:"topRight"})}finally{C(),w.disabled=!1}})},onClose:s=>{document.body.style.overflow="visible",C(),s!=null&&s._onEscClose&&document.removeEventListener("keydown",s._onEscClose)}});h==null||h.close(),B(),t.show()}//!=============================================================
let r=1,c=1,g=8,z="",y=[];//!=============================================================
async function ve(){const e=await re(),t=["Собаки","Коти","Кролики","Гризуни","Птахи","Тварини з особливими потребами","Терміново шукають дім"],s=[...e].sort((o,n)=>{const i=t.indexOf(o.name),l=t.indexOf(n.name);return i===-1?1:l===-1?-1:i-l});be(s),window.innerWidth>=1440?g=9:g=8,await d()}//!=============================================================
...
function be(e){const s=[{name:"Всі",_id:""},...e].map(n=>`<li class="pets-filters-item"><button class="pets-filters-btn" data-category-id="${n._id}">${n.name}</button></li>`).join("");a.petsFilters.innerHTML=s;const o=document.querySelector(".pets-filters-btn");o&&o.classList.add("pets-filters-btn-active")}//!=============================================================
function ye(e,{append:t=!0}={}){const s=e.map(({_id:o,name:n,image:i,species:l,age:f,gender:u,shortDescription:w,categories:L})=>{const v=L.map(b=>`<p class="pet-category">${b.name}</p>`).join("");return`
        <li class="pet-card">
