...
function V(e){const t=e.map(({_id:r,name:n,image:s,species:o,age:a,gender:l,shortDescription:c,categories:k})=>{const M=k.map(q=>`<p class="pet-category">${q.name}</p>`).join("");return`
        <li class="pet-card">
          <img class="pet-img" src="${s}" alt="${n}" />
          <div class="pet-info">
            <p class="pet-type">${o}</p>
            <h3 class="pet-name">${n}</h3>
            <div class="pet-categories">${M}</div>
            <ul class="pet-meta">
              <li>${a}</li>
              <li>${l}</li>
            </ul>
...
async function y(){Y();try{const{animals:e,totalItems:t}=await x({page:d,limit:b,categoryId:E});if(S.push(...e),e.length===0){u();return}V(e),v=Math.ceil(t/b),d<v?G():u(),d++}catch(e){console.error("API error:",e)}finally{z()}}//!===============================================================================
